...
export default function Player({ songId, setSongId, playlist }) {
	...
	useEffect(() => {
		if (audioData == null || audioData.error) {
			return;
		}
		...
		if ('mediaSession' in navigator) {
			...
			navigator.mediaSession.setActionHandler('nexttrack', function () {
				nextSong(songId, setSongId, playlist);
			});
	...
	const handleProgressClick = (e) => {
		...
		const newTime = (clickedPosition / progressRef.current.offsetWidth) * audioData.track.duration;
	...
	const nextSong = (songId, setSongId, playlist) => {
		...
		if (currentIndex === -1) {
			console.error("Le morceau actuel n'existe pas dans la liste de lecture.");
			return null;
		}
	...
	if (isLoading) {
		return (
			<div className="flex justify-start items-center w-full absolute left-0 bottom-0 overflow-hidden md:gap-28 px-4 py-3.5 bg-[#1f1f22] z-20">
				<div className="flex justify-start items-center self-stretch flex-grow-0 flex-shrink-0 relative overflow-hidden gap-4">
					<audio ref={audioRef} src={audioData ? audioData.track.src : ''} onTimeUpdate={handleTimeUpdate}></audio>
					...
					<div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 relative gap-1.5">
						<div className="flex justify-start items-center flex-grow-0 flex-shrink-0 relative gap-2">
							...
							<div className="flex justify-start items-start flex-grow-0 flex-shrink-0 w-4 h-4 relative overflow-hidden gap-2.5">
								<svg width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg" className="self-stretch flex-grow relative" preserveAspectRatio="none">
									<path d="M2.30001 8.96658C1.00001 7.56658 1.00001 5.29991 2.30001 3.89991C2.96668 3.16658 3.80001 2.83325 4.66668 2.83325C5.33335 2.83325 6.03334 3.06658 6.60001 3.49991C6.83334 3.69991 7.03335 3.93325 7.23335 4.16658L8.03335 5.23325L8.83334 4.16658C9.03334 3.89991 9.23334 3.66658 9.46668 3.49991C10 3.03325 10.6667 2.83325 11.3667 2.83325C12.2 2.83325 13.0667 3.16658 13.7 3.89991C15 5.29991 15 7.56658 13.7 8.96658L8.46668 14.2999C8.23334 14.5666 7.80001 14.5666 7.56668 14.2999L2.30001 8.96658Z" stroke="#FCFCFC" stroke-width="2" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path>
								</svg>
							</div>
							<div className="flex justify-start items-start flex-grow-0 flex-shrink-0 w-4 h-4 relative overflow-hidden gap-2.5">
								<svg width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg" className="self-stretch flex-grow relative" preserveAspectRatio="none">
						...
						<p className="flex-grow-0 flex-shrink-0 w-[139px] h-3 text-[10px] font-semibold text-left uppercase text-[#fcfcfc]/[0.65]">PLAING FROM: {audioData ? audioData.playlist : ''}</p>
					</div>
				</div>
				<div className="flex flex-col justify-center items-center flex-grow overflow-hidden gap-4 px-8">
					...
					<div className="flex justify-start items-center self-stretch flex-grow-0 flex-shrink-0 relative overflow-hidden gap-8">
						...
						<div onClick={handleProgressClick} ref={progressRef} className="flex-grow w-[515px] h-1 relative overflow-hidden rounded-[999px] bg-[#4c4e54]">
							<div className="absolute top-0 left-0 h-full bg-blue-500" style={{ width: `0` }}></div>
						</div>
				...
				<div className="flex justify-end items-center flex-grow-0 flex-shrink-0 relative overflow-hidden gap-8 px-8">
					...
					<div className="flex justify-start items-start flex-grow-0 flex-shrink-0 w-6 h-6 relative overflow-hidden gap-2.5">
						<svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg" className="self-stretch flex-grow relative" preserveAspectRatio="none">
							...
							<path d="M3 5.5H21" stroke="#FCFCFC" stroke-width="2" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path>
							<g opacity="0.6">
								<path d="M3 19.5H21H3Z" fill="#FCFCFC"></path>
								<path d="M3 19.5H21" stroke="#FCFCFC" stroke-width="2" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path>
							</g>
							<g opacity="0.6">
								<path d="M3 12.5H21H3Z" fill="#FCFCFC"></path>
								<path d="M3 12.5H21" stroke="#FCFCFC" stroke-width="2" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path>
							</g>
						</svg>
					</div>
					<div className="flex justify-center items-center flex-grow-0 flex-shrink-0 p-3 rounded-lg bg-[#1f1f22] border border-[#ebebff]/5">
						<div className="flex justify-start items-start flex-grow-0 flex-shrink-0 w-4 h-4 relative overflow-hidden gap-2.5">
							<svg width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg" className="self-stretch flex-grow relative" preserveAspectRatio="none">
								<path d="M13.2 10.5L8.00001 5.30005L2.80001 10.5" stroke="#FCFCFC" stroke-width="2" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path>
							</svg>
						</div>
					</div>
				</div>
	...
	return (
		<div className="flex justify-start items-center w-full absolute left-0 bottom-0 overflow-hidden md:gap-28 px-4 py-3.5 bg-[#1f1f22] z-20">
			<div className="flex justify-start items-center self-stretch flex-grow-0 flex-shrink-0 relative overflow-hidden gap-4">
				<audio ref={audioRef} src={audioData.track.src} onTimeUpdate={handleTimeUpdate}></audio>
				<img src={audioData.album.image} className="flex-grow-0 flex-shrink-0 w-16 h-[65px] object-cover mt-2" />
				<div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 relative gap-1.5">
					<div className="flex justify-start items-center flex-grow-0 flex-shrink-0 relative gap-2">
					...
					<p className="flex-grow-0 flex-shrink-0 w-[139px] h-3 text-[10px] font-semibold text-left uppercase text-[#fcfcfc]/[0.65]">PLAING FROM: {audioData.playlist}</p>
				</div>
			</div>
			<div className="flex flex-col justify-center items-center flex-grow overflow-hidden gap-4 px-8">
				<div className="flex justify-center items-center flex-grow-0 flex-shrink-0 overflow-hidden gap-8">
				...
				<div className="flex justify-start items-center self-stretch flex-grow-0 flex-shrink-0 relative overflow-hidden gap-8">
					<p className="flex-grow-0 flex-shrink-0 text-xs font-semibold text-center text-white">{`${Math.floor(currentTime / 60)
						.toString()
						.padStart(2, '0')}:${(currentTime % 60).toString().split('.')[0].padStart(2, '0')}`}</p>

					<div onClick={handleProgressClick} ref={progressRef} className="flex-grow w-[515px] h-1 relative overflow-hidden rounded-[999px] bg-[#4c4e54]">
						<div className="absolute top-0 left-0 h-full bg-blue-500" style={{ width: `${(currentTime / audioData.track.duration) * 100}%` }}></div>
					</div>

					<p className="flex-grow-0 flex-shrink-0 text-xs font-semibold text-center text-white">{`${Math.floor(audioData.track.duration / 60)
						.toString()
						.padStart(2, '0')}:${(audioData.track.duration % 60).toString().padStart(2, '0')}`}</p>
				</div>
			</div>
			<div className="flex justify-end items-center flex-grow-0 flex-shrink-0 relative overflow-hidden gap-8 px-8">
				<svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg" className="flex-grow-0 flex-shrink-0 w-6 h-6 relative" preserveAspectRatio="none">
