...
async function jioSaavnGetCall<T>(
  ...
  const url = new URL(path, process.env.JIOSAAVN_API_URL);
  url.search = new URLSearchParams(queries).toString();

  console.log(process.env.JIOSAAVN_API_URL+ path + url.search);
  

  const response = await fetch(process.env.JIOSAAVN_API_URL+ path + url.search, { cache: "force-cache" });
...
export async function searchAll(query: string) {
  return await jioSaavnGetCall<AllSearch>("/search", { query: query });
}
